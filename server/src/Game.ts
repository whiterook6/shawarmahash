...
export class Game {
  ...
  public getPlayerBlocks = (player: string): Block[] => {
    return this.chain.filter(block => block.player === player).map(block => {
      return {
        ...block
      } as Block;
    })
  ...
  public getTeamBlocks = (team: string)  => {
    return this.chain.filter(block => block.team === team).map(block => {
      return {
        ...block
      } as Block;
    });
  ...
  public getBlockAt = (height: number) => {
    if (this.chain.length === 0){
      ...
      throw new Error(`Cannot find block. Requested block ${height}, but highest block is ${this.chain.length - 1}.`);
    }
